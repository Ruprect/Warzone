...
public class ASCENSION_GD extends Gamedata {
	...
	private Location sign1 = new Location(Maps.Warzone_World, 28.5, 73.62, 108.5),
	sign2 = new Location(Maps.Warzone_World, 30.5, 73.62, 106.5);
	...
	@Override
	public void saveData(ArrayList<Player> partic){
		for(Player p : partic){
	...
	@Override
	public void respawn() {
		participants.get(0).teleport(Warzone.mapList.get(MapName.ASCENSION).getSpawnPoints().get(0));
		participants.get(1).teleport(Warzone.mapList.get(MapName.ASCENSION).getSpawnPoints().get(1));
