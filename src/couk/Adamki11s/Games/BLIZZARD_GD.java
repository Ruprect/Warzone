...
public class BLIZZARD_GD extends Gamedata {
	...
	private Location sign1 = new Location(Maps.Warzone_World, -163.5, 69.62, 111.5),
	sign2 = new Location(Maps.Warzone_World, -184.5, 69.62, 116.5);
	...
	@Override public void endGame(int taskid){
		...
		shotsMissed.clear();
		timer = 0;
		Warzone.mapList.get(MapName.BLIZZARD).setOccupiedState(false);
	...
	@Override
	public void respawn() {
		participants.get(0).teleport(Warzone.mapList.get(MapName.BLIZZARD).getSpawnPoints().get(0));
		participants.get(1).teleport(Warzone.mapList.get(MapName.BLIZZARD).getSpawnPoints().get(1));
