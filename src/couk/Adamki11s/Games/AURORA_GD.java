...
public class AURORA_GD extends Gamedata {
	...
	private Location sign1 = new Location(Maps.Warzone_World, -307.3, 120.6, 233.3),
	sign2 = new Location(Maps.Warzone_World, -264.5, 120.6, 284.4);
	...
	@Override public void endGame(int taskid){
		...
		shotsMissed.clear();
		timer = 0;
		Warzone.mapList.get(MapName.AURORA).setOccupiedState(false);
	...
	@Override
	public void respawn() {
		participants.get(0).teleport(Warzone.mapList.get(MapName.AURORA).getSpawnPoints().get(0));
		participants.get(1).teleport(Warzone.mapList.get(MapName.AURORA).getSpawnPoints().get(1));
