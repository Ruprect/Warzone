...
public class DOME_GD extends Gamedata {
	...
	@Override public void endGame(int taskid){
		...
		shotsMissed.clear();
		timer = 0;
		Warzone.mapList.get(MapName.DOME).setOccupiedState(false);
	...
	@Override
	public void respawn() {
		participants.get(0).teleport(Warzone.mapList.get(MapName.DOME).getSpawnPoints().get(0));
		participants.get(1).teleport(Warzone.mapList.get(MapName.DOME).getSpawnPoints().get(1));
