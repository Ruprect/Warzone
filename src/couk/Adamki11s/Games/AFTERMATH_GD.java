...
public class AFTERMATH_GD extends Gamedata {
	...
	private Location sign1 = new Location(Maps.Warzone_World, 197.69, 52.62, -54.57),
	sign2 = new Location(Maps.Warzone_World, 240.3, 52.62, -53.5);
	...
	@Override public void endGame(int taskid){
		...
		shotsMissed.clear();
		timer = 0;
		Warzone.mapList.get(MapName.AFTERMATH).setOccupiedState(false);
	...
	@Override
	public void respawn() {
		participants.get(0).teleport(Warzone.mapList.get(MapName.AFTERMATH).getSpawnPoints().get(0));
		participants.get(1).teleport(Warzone.mapList.get(MapName.AFTERMATH).getSpawnPoints().get(1));
